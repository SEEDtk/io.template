...
class TestPatterns {
    ...
    void test() {
        ...
        m = JsonConverter.GENOME_KEY_PATTERN.matcher("patric_id_A");
        assertThat(m.matches(), equalTo(false));
        m = JsonConverter.FEATURE_KEY_PATTERN.matcher("patric_id");
        assertThat(m.matches(), equalTo(true));
        assertThat(m.group(1), equalTo("patric_id"));
        assertThat(m.group(2), nullValue());
        m = JsonConverter.FEATURE_KEY_PATTERN.matcher("interactor");
        assertThat(m.matches(), equalTo(true));
        assertThat(m.group(1), equalTo("interactor"));
        assertThat(m.group(2), nullValue());
        m = JsonConverter.FEATURE_KEY_PATTERN.matcher("interactor_a");
        assertThat(m.matches(), equalTo(true));
        assertThat(m.group(1), equalTo("interactor"));
        assertThat(m.group(2), equalTo("_a"));
        m = JsonConverter.FEATURE_KEY_PATTERN.matcher("interactor_b");
        assertThat(m.matches(), equalTo(true));
        assertThat(m.group(1), equalTo("interactor"));
